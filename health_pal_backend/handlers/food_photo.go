...
func UploadFoodPhotoHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := models.CreateFoodPhoto(db, &foodPhoto); err != nil {
...
func GetFoodPhotoHistoryHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID, exists := c.Get("userID")
		if !exists {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Unauthorized"})
		...
		foodPhotos, err := models.GetFoodPhotosByUserID(db, userID.(uint))
		if err != nil {
