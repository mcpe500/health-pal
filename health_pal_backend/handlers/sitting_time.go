...
// SittingTimeHandler struct for handling sitting time related requests
type SittingTimeHandler struct {
	SittingTimeModel *models.SittingTimeModel
}
...
func (h *SittingTimeHandler) RecordSittingTimeHandler(c *gin.Context) {
	...
	sittingTime, err := h.SittingTimeModel.GetSittingTimeByUserIDAndDate(userID.(uint), parsedDate)
	...
	if sittingTime != nil {
		...
		err = h.SittingTimeModel.UpdateSittingTime(sittingTime)
		if err != nil {
	...
	err = h.SittingTimeModel.CreateSittingTime(&newSittingTime)
	if err != nil {
...
func (h *SittingTimeHandler) GetSittingTimeHistoryHandler(c *gin.Context) {
	userID, exists := c.Get("userID")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Unauthorized"})
		return
	...
	sittingTimes, err := h.SittingTimeModel.GetSittingTimesByUserID(userID.(uint))
	if err != nil {
